...
public class GUITipoTratamento extends JDialog {
    ...
    private TipoTratamentoTableModel tableModel;
    DAOTipoTratamento daoTipoTratamento = new DAOTipoTratamento();

    public GUITipoTratamento() {

        setTitle("CRUD - TipoTratamento");
        ...
        List<TipoTratamento> lista = new ArrayList<>();
        tableModel = new TipoTratamentoTableModel(lista);
...
//        painelBotoes.add(btnRem);
            ...
            public void actionPerformed(ActionEvent evt) {
                if (table.getSelectedRow() >= 0) {
                    ...
                    if (JOptionPane.YES_OPTION == JOptionPane.showConfirmDialog(cp,
                            "Confirme a exclusão [" + tableModel.getValue(table.getSelectedRow()).getIdTipoTratamento() + " - "
                            + tableModel.getValue(table.getSelectedRow()).getNomeTipoTratamentocol()+ "]?", "Confirm",
        ...
        btnAdd.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                tipoTratamento.setIdTipoTratamento(daoTipoTratamento.autoIdTipoTratamento());
                tipoTratamento.setNomeTipoTratamentocol("Nome");

                try {
                    TipoTratamento c = daoTipoTratamento.obter(tipoTratamento.getIdTipoTratamento());
                    if (c == null) {
                        daoTipoTratamento.inserir(tipoTratamento);
                        tableModel.onAdd(tipoTratamento);
                        tableModel.fireTableDataChanged();
                    } else {
                        JOptionPane.showMessageDialog(cp, "Já existe");
                    }
                } catch (Exception err) {
                    JOptionPane.showMessageDialog(cp, "Erro ao inserir =>" + tipoTratamento.getIdTipoTratamento()
        ...
        btnRem.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (table.getSelectedRow() != -1 && table.getSelectedRow() < tableModel.getRowCount()) {
                    TipoTratamento c = tableModel.getValue(table.getSelectedRow());
                    daoTipoTratamento.remover(c);
                    tableModel.onRemove(table.getSelectedRows());
                } else {
                    JOptionPane.showMessageDialog(cp, "Escolha na tabela a tipoTratamento a ser excluída");
                    table.requestFocus();
                }
                tableModel.fireTableDataChanged();
            }
        ...
        btnCarregar.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    List<TipoTratamento> lc = daoTipoTratamento.list();
